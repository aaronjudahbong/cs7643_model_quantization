...
if __name__ == "__main__":
    ...
    # Inspect sample weights after training
    print("\n" + "="*80)
    print("INSPECTING MODEL WEIGHTS (After Training, Before Conversion)")
    print("="*80)
    print("NOTE: Model is in QAT mode, so weights are FP32 with fake quantization")
    print()
    ...
    for name, param in prepared_model.named_parameters():
        if param.requires_grad and layer_count < num_layers_to_show:
            print(f"Layer: {name}")
            print(f"  Shape: {param.shape}")
            print(f"  Dtype: {param.dtype}")
            
            # Check if quantized (unlikely in QAT mode, but check anyway)
            if hasattr(param, 'int_repr'):
                print(f"  Type: Quantized (qint8/quint8)")
                try:
                    scale = param.q_scale().item() if hasattr(param, 'q_scale') else 'N/A'
                    zero_point = param.q_zero_point().item() if hasattr(param, 'q_zero_point') else 'N/A'
                    print(f"  Scale: {scale}")
                    print(f"  Zero Point: {zero_point}")
                    int_vals = param.int_repr()
                    print(f"  Integer values (first {num_samples}): {int_vals.flatten()[:num_samples].tolist()}")
                except:
                    pass
            else:
                # FP32 parameter (expected in QAT mode)
                print(f"  Type: FP32 (QAT mode - fake quantization applied during forward)")
                sample_vals = param.data.flatten()[:num_samples]
                print(f"  Values (first {num_samples}): {[f'{v:.6f}' for v in sample_vals.tolist()]}")
            ...
            # Show assigned bit depth if available
            if name in layer_bit_depths:
                print(f"  Assigned Bit Depth: {layer_bit_depths[name]}-bit")
            else:
                # Try to match by module name
                for layer_name, bit_depth in layer_bit_depths.items():
                    if layer_name in name or name.endswith(layer_name.split('.')[-1]):
                        print(f"  Assigned Bit Depth: {bit_depth}-bit")
                        break
    ...
    if layer_count >= num_layers_to_show:
        print(f"... (showing first {num_layers_to_show} layers)")
    print("="*80)
    print()
    
    # Scan all weight tensors for FP32 vs quantized
    print("="*80)
    print("SCANNING ALL WEIGHT TENSORS FOR FP32 VALUES")
    print("="*80)
    ...
    for name, param in prepared_model.named_parameters():
        if param.requires_grad:
            ...
            if is_quantized:
                ...
                quantized_param_count += 1
                total_quantized_params += param.numel()
            else:
                fp32_layers.append(name)
                fp32_param_count += 1
                total_fp32_params += param.numel()
    
    print(f"Total layers with parameters: {fp32_param_count + quantized_param_count}")
    print(f"  FP32 layers: {fp32_param_count}")
    print(f"  Quantized layers: {quantized_param_count}")
    print()
    print(f"Total parameters:")
    print(f"  FP32 parameters: {total_fp32_params:,} ({total_fp32_params * 4 / 1024 / 1024:.2f} MB)")
    print(f"  Quantized parameters: {total_quantized_params:,} ({total_quantized_params / 1024 / 1024:.2f} MB)")
    print()
    
    # Check if layers that should be quantized are still FP32
    if layer_bit_depths:
        print("Checking layers assigned bit depths vs actual quantization status:")
        print("-" * 80)
        
        layers_should_be_quantized = []
        layers_correctly_fp32 = []
        
        for layer_name, bit_depth in layer_bit_depths.items():
            # Find matching parameter
            found = False
            for param_name, param in prepared_model.named_parameters():
                if layer_name in param_name or param_name.endswith(layer_name.split('.')[-1]):
                    is_quantized = hasattr(param, 'int_repr')
                    if not is_quantized:
                        layers_should_be_quantized.append((layer_name, bit_depth, param_name))
                    else:
                        layers_correctly_fp32.append((layer_name, bit_depth, param_name))
                    found = True
                    break
        
        if layers_should_be_quantized:
            print(f"WARNING: {len(layers_should_be_quantized)} layers assigned bit depths but still FP32:")
            for layer_name, bit_depth, param_name in layers_should_be_quantized[:10]:
                print(f"  {layer_name} (assigned {bit_depth}-bit) -> {param_name}")
            if len(layers_should_be_quantized) > 10:
                print(f"  ... and {len(layers_should_be_quantized) - 10} more")
        else:
            print("All layers with assigned bit depths are properly configured.")
        
        print()
        print(f"Layers correctly in FP32 (no bit depth assigned or skipped): {len(layers_correctly_fp32)}")
    
    # Show sample FP32 layers
    if fp32_layers:
        print(f"\nSample FP32 layers (first 10):")
        for layer_name in fp32_layers[:10]:
            print(f"  {layer_name}")
        if len(fp32_layers) > 10:
            print(f"  ... and {len(fp32_layers) - 10} more FP32 layers")
    
    # Show quantized layers if any
    if quantized_layers:
